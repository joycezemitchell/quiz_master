...
func UpdateQuestion(arrCommandStr []string) error {
	...
	if q2 == (q.Question{}) {
		return errors.New(fmt.Sprintf("Question no %v does not exist", i))
	}
	...
	err := services.QuestionService.UpdateQuestion(*request)
	...
	fmt.Println(fmt.Sprintf("Question no %v was updated", id))
